---
title: "staggered-level_MSIBSSD_BayesianD_type2_functions"
...
# supplot are primary
```{r}
evaluateMBayesianD <- function(MSSD,primary.number = 5, U2, b1 ,n2){
  tau <- sqrt(5)
  m <- ncol(MSSD)
  
  #primary
  primary <- cbind(1,MSSD[,(m1+m2+1):(m1+m2+primary.number)])
  p1 <- ncol(primary)
  
  #potential
  if(primary.number == m3){
    potential <- MSSD[,c(1:(m1+m2))]
  }else{
    potential <- MSSD[,c(1:(m1+m2), (m1+m2+primary.number+1):ncol(MSSD))]  
  }
  
  p2 <- ncol(potential)
  potential.adj <- potential *0.5   
...
evaluateMBayesianD <- function(MSSD, U2, b1 ,n2){
  ...
  #combine
  primary <- cbind(1, primary.m1, primary.m2, primary.m3)
  p1 <- ncol(primary)
  ...
  #m3
  potential.m3 <- MSSD[,(m1+m2+b1):(m1+m2+m3)]
...
#w1, w2 first three are primary, s all are primary
```{r}
evaluateMBayesianD <- function(MSSD, U2, b1 ,n2){
  tau <- sqrt(5)
  m <- ncol(MSSD)
  ...
  #m3
  primary.m3 <- MSSD[,(m1+m2+1):(m1+m2+m3)]
  ...
  #combine
  potential <-cbind(potential.m1, potential.m2)
...
evaluateMBayesianD <- function(MSSD, U2, b1 ,n2){
  ...
  #m2
  potential.m2 <- MSSD[,(m1+b1):(m1+m2+m3)]
...
# primary intercept, interaction & others are potential
```{r}
evaluateMBayesianD <- function(MSSD, U2, b1 ,n2){
  tau <- sqrt(1)
  m <- ncol(MSSD)
...
# radom
```{r}
mixOperation_type2 <- function(particle, U2, m1, m2, m3, b1, n2, mSIBSSD.list, w.ratio = 0.1, LB, GB, qLB, qGB){
  mixwLB <- list()
  mixwGB <- list()
  for(i in 1:particle){
    ...
    if(w.qLB[1] != 0){
      for(j in 1:w.qLB[1]){
        delete.index <- sample(m1,1)
        tem <- tem[,-delete.index]
    ...
    if(w.qLB[2] != 0){
      for(j in 1:w.qLB[2]){
        delete.index <- sample(m2,1)
        tem <- tem[,-(m1 + delete.index)]
    ...
    if(s.qLB != 0){
      for(j in 1:s.qLB){
        delete.index <- sample(m3,1)
        tem <- tem[,-(m1 + m2 + delete.index)]
